...
DM2d split axis: one component / module per file under
`src/model/`.

## Block-diagram contract

This is the highest-risk failure mode in DM2d: the structural
gate is a literal-text grep, so renaming `Top` or re-exporting
it from elsewhere makes the gate fail even though the code
compiles. Keep this section's contract intact verbatim.

The orchestrator auto-renders the block diagram on the
DM2d -> DM3a advance via `crate::dump_topology(&args)` defined
in `src/lib.rs`. The pieces DM2d MUST preserve:
...
- `dump_topology` in `src/lib.rs` stays callable with
  `&TopologyDumpArgs`. Do not rename, delete, or change its
  signature.
...
structural grep checks `src/model/top.rs` for the literal text
`pub struct Top` AND `impl Module for Top`. A type alias or
re-export does not contain those tokens; the gate fails even
though the code compiles. If you want a descriptive struct
name internally, define it as the inner field of `Top`, not
as `Top` itself.
...
under `src/model/`; writes to `src/<file>.rs` for code the
plan placed under `src/model/` are auto-redirected by the
orchestrator (you'll see a notice in the tool result) -- but
the right move is to write the canonical path the first time.

Replace the stub `Top` body with the real model -- add fields,
child modules, port wiring, evaluate/settle/update bodies --
but keep the type name + `Default::default()` constructibility
so the orchestrator's auto-render keeps working through the
entire flow.
